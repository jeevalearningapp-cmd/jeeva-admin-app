...
import { CreateUserSubscriptionInput, UpdateUserSubscriptionInput } from '@/types/subscription'
...
export const useCreateUserSubscription = () => {
  ...
  return useMutation({
    mutationFn: (input: CreateUserSubscriptionInput) => userSubscriptionsAPI.create(input),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['user-subscriptions'] })
      enqueueSnackbar('Subscription created successfully', { variant: 'success' })
    },
    onError: (error: any) => {
      enqueueSnackbar(error.message || 'Failed to create subscription', { variant: 'error' })
    },
  })
}
...
export const useUpdateUserSubscription = () => {
  ...
  return useMutation({
    mutationFn: ({ id, input }: { id: string; input: UpdateUserSubscriptionInput }) =>
    ...
    onError: (error: any) => {
      enqueueSnackbar(error.message || 'Failed to update subscription', { variant: 'error' })
    },
  })
}
...
export const useDeleteUserSubscription = () => {
  ...
  return useMutation({
    ...
    onError: (error: any) => {
      enqueueSnackbar(error.message || 'Failed to delete subscription', { variant: 'error' })
    },
  })
}
