...
export const useBulkUpload = () => {
  ...
  const uploadQuestions = async (
    ...
    try {
      ...
      for (const item of data) {
        ...
        if (
          ...
          for (let i = 1; i <= 4; i++) {
            ...
            const isCorrect =
              item[`option_${i}_correct`]?.toLowerCase() === "true";
      ...
      if (questionsError) {
        throw new Error(
          `Failed to insert questions: ${questionsError.message}`,
        );
      }
      ...
      if (optionsData.length > 0) {
        ...
        if (optionsError) {
          ...
          if (deleteError) {
            console.error(
              "Rollback failed - orphaned questions:",
              questionIds,
              deleteError,
            );
            throw new Error(
      ...
      enqueueSnackbar(
        `Successfully uploaded ${questionsData.length} questions with ${optionsData.length} options`,
        {
          variant: "success",
        },
      );
