    ...
    onSuccess: (data) => {
      ...
      if (data.lessonId) {
        queryClient.invalidateQueries({
          queryKey: ["questions", "lesson", data.lessonId],
        });
      }
...
export const useBulkCreateQuestions = () => {
  ...
  return useMutation({
    mutationFn: (inputs: CreateQuestionInput[]) =>
      questionsAPI.bulkCreate(inputs),
    onSuccess: (data) => {
      queryClient.invalidateQueries({ queryKey: ["questions"] });
      enqueueSnackbar(`${data.length} questions created successfully`, {
        variant: "success",
      });
    },
    onError: (error: any) => {
      enqueueSnackbar(error.message || "Failed to create questions", {
        variant: "error",
      });
    },
  });
