...
export const notificationsAPI = {
  ...
  async getNotificationStats(id: string): Promise<NotificationStats> {
    ...
    if (error) {
      ...
      const totalSent = targets.filter((t) =>
        ["sent", "delivered", "read"].includes(t.deliveryStatus),
      ).length;
      const totalDelivered = targets.filter((t) =>
        ["delivered", "read"].includes(t.deliveryStatus),
      ).length;
      const totalFailed = targets.filter(
        (t) => t.deliveryStatus === "failed",
      ).length;
      const totalRead = targets.filter(
        (t) => t.deliveryStatus === "read",
      ).length;
    ...
    return {
      ...
      openRate:
        stats.total_delivered > 0
          ? (stats.total_read / stats.total_delivered) * 100
          : 0,
    };
  },
  ...
  async getUserTargetingOptions(): Promise<UserTargetingOption[]> {
    ...
    try {
      // Use the same RPC function that StudentsPage uses - it works!
      const { data: students, error: rpcError } = await supabase.rpc(
        "get_student_details",
      );

      if (rpcError) {
        console.error("Error fetching student details:", rpcError);
        throw rpcError;
      }
      ...
      const subscribedUsers =
        students?.filter((s: any) => s.subscription_status === "active")
          .length || 0;

      const nonSubscribedUsers = totalUsers - subscribedUsers;

      // Add all three targeting options
      options.push({
        id: "all",
        label: "All Users",
        count: totalUsers,
      ...
      // Return at least the "All Users" option with 0 count
      return [
        {
          id: "all",
          label: "All Users",
  ...
  async updateNotificationPreferences(
    ...
    if (preferences.subscriptionExpiringEnabled !== undefined)
      updateData.subscription_expiring_enabled =
        preferences.subscriptionExpiringEnabled;
    if (preferences.contentApprovedEnabled !== undefined)
      updateData.content_approved_enabled = preferences.contentApprovedEnabled;
    if (preferences.welcomeEnabled !== undefined)
      updateData.welcome_enabled = preferences.welcomeEnabled;
    if (preferences.milestonesEnabled !== undefined)
      updateData.milestones_enabled = preferences.milestonesEnabled;
    if (preferences.marketingEnabled !== undefined)
      updateData.marketing_enabled = preferences.marketingEnabled;
  ...
  async sendDirectPush(input: {
    ...
    const response = await fetch("/api/notifications/direct-push", {
      method: "POST",
      headers: {
    ...
    if (!response.ok) {
      ...
      throw new Error(
        errorData.error ||
          `HTTP ${response.status}: Failed to send direct push`,
      );
    }
