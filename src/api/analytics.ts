...
export const analyticsAPI = {
  async getAnalytics(dateRange?: DateRange): Promise<AnalyticsData> {
    try {
      ...
      const endDate =
        dateRange?.endDate || formatISO(new Date(), { representation: "date" });
      ...
      try {
        const { data, error } = await supabase
          ...
          .select(
            "*, lessons(id, title, topics(id, title, modules(id, title)))",
          )
        ...
        if (!error && data) completions = data;
          ...
          console.warn(
            "Learning completions query failed (table may not exist):",
            error.message,
          );
      } catch (err) {
      ...
      const activeUsers =
        users?.filter(
          (u) =>
            u.last_login && new Date(u.last_login) > subDays(new Date(), 7),
        ).length || 0;
      ...
      const moduleViews: Record<
        string,
        { name: string; views: number; completions: number }
      > = {};
      ...
      const topModules = Object.entries(moduleViews)
        .map(([id, data]) => ({
          ...
          completionRate:
            data.views > 0 ? (data.completions / data.views) * 100 : 0,
        }))
      ...
      return {
        ...
        contentPerformance: {
          ...
          totalCompletions:
            completions?.filter((c: any) => c.is_completed).length || 0,
          averageCompletionRate:
            topModules.length > 0
              ? topModules.reduce((sum, m) => sum + m.completionRate, 0) /
                topModules.length
              : 0,
          topModules,
          topLessons: [],
          ...
          subscriptionBreakdown: [
            {
              planType: "Premium",
              count:
                subscriptions?.filter((s: any) => s.plan_type === "premium")
                  .length || 0,
              revenue: 0,
            },
            {
            ...
            },
            {
              planType: "Trial",
              count:
                subscriptions?.filter((s: any) => s.plan_type === "trial")
                  .length || 0,
              revenue: 0,
            },
          ],
        ...
        engagementTrends: Array.from({ length: 7 }, (_, i) => ({
          date: formatISO(subDays(new Date(), 6 - i), {
            representation: "date",
          }),
          activeUsers: 0,
          sessions: 0,
  ...
  async exportAnalyticsCSV(data: AnalyticsData): Promise<string> {
    ...
    const rows = [
      ...
      [
        "Content Completions",
        data.contentPerformance.totalCompletions.toString(),
      ],
      [
        "Avg Completion Rate",
        `${data.contentPerformance.averageCompletionRate.toFixed(2)}%`,
      ],
    ];
