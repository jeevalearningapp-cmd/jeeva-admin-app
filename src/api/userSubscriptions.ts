...
const mapToUserSubscription = (data: any): UserSubscription => ({
  ...
  plan: data.subscription_plans
    ? {
        ...
        price:
          data.subscription_plans.price_usd ||
...
export const userSubscriptionsAPI = {
  async getAll(): Promise<UserSubscription[]> {
    const { data, error } = await supabase
      .from("subscriptions")
      .select(
        "*, users(id, email, first_name, last_name), subscription_plans(*)",
      )
  ...
  async update(
    ...
    if (input.lastPaymentDate !== undefined)
      updateData.last_payment_date = input.lastPaymentDate;
  ...
  async getAnalytics(): Promise<SubscriptionAnalytics> {
    ...
    const planBreakdown: Record<
      string,
      { planId: string; planName: string; count: number; revenue: number }
    > = {};
