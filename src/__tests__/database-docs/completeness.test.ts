...
describe('Property 1: Documentation Completeness', () => {
  ...
  it('should have documentation for all Authentication & Users tables', () => {
    ...
    fc.assert(
      fc.property(
        fc.constantFrom(...AUTH_USERS_TABLES),
        (table) => {
          // Check that the table name appears as a heading (### table_name)
          const tableHeadingPattern = new RegExp(`###\\s+\\d+\\.\\d+\\s+${table.name}`, 'i');
          const hasTableHeading = tableHeadingPattern.test(schemaDocContent);
          
          // Check that the table has a Purpose description
          const purposePattern = new RegExp(`\\*\\*Purpose:\\*\\*.*${table.name}`, 'is');
          const hasPurpose = schemaDocContent.includes(`### 1.`) && 
    ...
    fc.assert(
      fc.property(
        ...
        (tableName) => {
          ...
          const isDocumented = schemaDocContent.toLowerCase().includes(tableName);
          
          // The table must have a section with columns
          const hasColumnSection = schemaDocContent.includes('| Column |') && 
