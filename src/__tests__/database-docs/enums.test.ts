...
describe("Property 8: Enum Value Completeness", () => {
  beforeAll(() => {
    ...
    const dataTypesPath = path.resolve(
      __dirname,
      "../../../docs/03-Database/DATA_TYPES.md",
    );
  ...
  it("property: any enum column should have all its values documented", () => {
    fc.assert(
      fc.property(fc.constantFrom(...ENUM_COLUMNS), (enumCol) => {
  ...
  it("property: enum documentation should include table and column context", () => {
    fc.assert(
      fc.property(fc.constantFrom(...ENUM_COLUMNS), (enumCol) => {
        ...
        const hasTable = dataTypesDocContent
          .toLowerCase()
          .includes(enumCol.table.toLowerCase());

        // Check that the column name is documented
        const hasColumn = dataTypesDocContent
          .toLowerCase()
          .includes(enumCol.column.toLowerCase());
  ...
  it("should document all Authentication & Users domain enums", () => {
    ...
    authEnums.forEach((enumCol) => {
      ...
      enumCol.values.forEach((value) => {
        expect(dataTypesDocContent.toLowerCase()).toContain(
          value.toLowerCase(),
        );
      });
    });
    ...
    fc.assert(
      fc.property(fc.constantFrom(...ENUM_COLUMNS), (enumCol) => {
        ...
        return enumCol.values.every((value) => {
          ...
          return (
            dataTypesDocContent.includes(quotedValue) ||
