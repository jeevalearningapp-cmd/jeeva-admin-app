...
const LEARNING_CONTENT_TABLES: TableSchema[] = [
  {
    ...
    columns: [
      ...
      { name: 'created_at', type: 'TIMESTAMP', nullable: 'YES', default: 'NOW()' },
      { name: 'updated_at', type: 'TIMESTAMP', nullable: 'YES', default: 'NOW()' },
    ],
  },
  {
    name: 'topics',
  ...
  },
  {
    ...
    columns: [
      ...
      { name: 'is_trial', type: 'BOOLEAN', nullable: 'YES', default: 'false' },
      { name: 'display_order', type: 'INTEGER', nullable: 'YES', default: '0' },
      { name: 'unlock_threshold', type: 'INTEGER', nullable: 'YES', default: '0' },
      { name: 'created_at', type: 'TIMESTAMP', nullable: 'YES', default: 'NOW()' },
      { name: 'updated_at', type: 'TIMESTAMP', nullable: 'YES', default: 'NOW()' },
    ],
  },
  ...
  },
  {
    ...
    columns: [
      ...
      { name: 'is_correct', type: 'BOOLEAN', nullable: 'YES', default: 'false' },
      { name: 'display_order', type: 'INTEGER', nullable: 'YES', default: '0' },
      { name: 'created_at', type: 'TIMESTAMP', nullable: 'YES', default: 'NOW()' },
    ],
  },
  ...
  },
  {
    ...
    columns: [
      ...
      { name: 'min_subscription_days', type: 'INTEGER', nullable: 'YES', default: null },
      { name: 'is_active', type: 'BOOLEAN', nullable: 'YES', default: 'true' },
      { name: 'created_at', type: 'TIMESTAMP', nullable: 'YES', default: 'NOW()' },
      { name: 'updated_at', type: 'TIMESTAMP', nullable: 'YES', default: 'NOW()' },
    ],
...
describe('Property 2: Column Detail Accuracy', () => {
    ...
    fc.assert(
      fc.property(
        ...
        ({ tableName, column }) => {
          ...
          return documentedDefault.includes(expectedDefault) || 
  ...
  it('property: column documentation should include all required fields', () => {
    const requiredColumnFields = ['Column', 'Type', 'Nullable', 'Default', 'Description'];
  ...
  it('should verify enum constraints are documented for content_type column', () => {
    const tableSection = extractTableSection(schemaDocContent, 'lesson_content');
    expect(tableSection).toBeTruthy();
    
    // Check that content_type enum values are documented
    const expectedEnumValues = ['text', 'image', 'video', 'audio', 'code', 'quiz'];
  ...
  it('should verify enum constraints are documented for question_type column', () => {
    ...
    const expectedEnumValues = ['multiple_choice', 'true_false', 'short_answer'];
    expectedEnumValues.forEach(value => {
      expect(tableSection?.toLowerCase()).toContain(value);
    });
  });
  ...
  it('should verify enum constraints are documented for access_type column', () => {
    const tableSection = extractTableSection(schemaDocContent, 'module_access_rules');
    expect(tableSection).toBeTruthy();
