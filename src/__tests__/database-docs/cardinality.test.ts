...
describe('Property 7: Relationship Cardinality', () => {
  beforeAll(() => {
    const erDiagramPath = path.resolve(__dirname, '../../../docs/03-Database/ER_DIAGRAMS.md');
  ...
  it('property: any documented relationship should have cardinality notation in ER diagram', () => {
    fc.assert(
      fc.property(
        fc.constantFrom(...DOCUMENTED_RELATIONSHIPS),
        (rel) => {
          // Check that the relationship appears in the ER diagram
          const hasParentTable = erDiagramContent.toLowerCase().includes(rel.parentTable.toLowerCase());
          const hasChildTable = erDiagramContent.toLowerCase().includes(rel.childTable.toLowerCase());
  ...
  it('property: 1:N relationships should use ||--o{ notation', () => {
    const oneToManyRelationships = DOCUMENTED_RELATIONSHIPS.filter(r => r.cardinality === '1:N');
    
    fc.assert(
      fc.property(
        fc.constantFrom(...oneToManyRelationships),
        (rel) => {
          ...
          const hasProperNotation = relationshipPattern.test(erDiagramContent) || 
                                   reversePattern.test(erDiagramContent);
          
          // At minimum, both tables should be in the diagram
          const hasBothTables = erDiagramContent.toLowerCase().includes(rel.parentTable) &&
  ...
  it('property: 1:1 relationships should use ||--o| or ||--|| notation', () => {
    const oneToOneRelationships = DOCUMENTED_RELATIONSHIPS.filter(r => r.cardinality === '1:1');
    
    fc.assert(
      fc.property(
        fc.constantFrom(...oneToOneRelationships),
        (rel) => {
          // Check that both tables exist in the diagram
          const hasParentTable = erDiagramContent.toLowerCase().includes(rel.parentTable);
          const hasChildTable = erDiagramContent.toLowerCase().includes(rel.childTable);
    ...
    fc.assert(
      fc.property(
        ...
        (rel) => {
          ...
          const hasOnDelete = erDiagramContent.includes('ON DELETE') ||
    ...
    fc.assert(
      fc.property(
        ...
        (rel) => {
          ...
          const parentInDiagram = erDiagramContent.toLowerCase().includes(rel.parentTable);
          const childInDiagram = erDiagramContent.toLowerCase().includes(rel.childTable);
