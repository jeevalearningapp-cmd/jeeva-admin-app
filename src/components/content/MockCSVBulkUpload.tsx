...
import { CreateMockQuestionInput } from "@/api/mockQuestions";
import { useBulkCreateMockQuestions } from "@/hooks/useMockQuestions";
import Papa from "papaparse";

interface MockCSVBulkUploadProps {
...
export const MockCSVBulkUpload: React.FC<MockCSVBulkUploadProps> = ({
  examPart,
}) => {
  const [parsedQuestions, setParsedQuestions] = useState<ParsedQuestion[]>([]);
  const [error, setError] = useState<string>("");
  const [success, setSuccess] = useState(false);
  const bulkCreateMutation = useBulkCreateMockQuestions();

  const handleDownloadTemplate = () => {
    const filename = `mock_exam_questions_template.csv`;
  ...
  const handleUpload = async () => {
    ...
    try {
      const questionInputs: CreateMockQuestionInput[] = parsedQuestions.map(
        (q) => ({
    ...
    <Box>
      <Box sx={{ textAlign: "center", py: 4 }}>
        ...
        <Typography variant="h6" gutterBottom>
          Bulk Upload Mock Exam Questions
        </Typography>
