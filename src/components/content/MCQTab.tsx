...
export const MCQTab: React.FC<MCQTabProps> = ({ subtopic, onAddQuestion, onEditQuestion }) => {
  ...
  const handleDelete = async (question: LearningQuestion) => {
    if (window.confirm(`Are you sure you want to delete this question?`)) {
      try {
        ...
        enqueueSnackbar(err.message || 'Failed to delete question', { variant: 'error' })
      }
    }
  }
    ...
    <Box>
      ...
      >
        {questions.length === 0 ? (
          <Box sx={{ p: 4, textAlign: 'center' }}>
            <QuizOutlined sx={{ fontSize: 48, color: 'action.disabled', mb: 2 }} />
      ...
      <Paper sx={{ p: 2, mt: 3, bgcolor: 'info.lighter', border: '1px solid', borderColor: 'info.light' }}>
        <Typography variant="body2" color="info.dark">
          <strong>Note:</strong> All questions are automatically mapped to this video lesson. Students
          must watch the video before taking the assessment. A passing score of 80% is required to
