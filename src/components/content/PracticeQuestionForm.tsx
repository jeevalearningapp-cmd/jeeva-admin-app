...
export const PracticeQuestionForm: React.FC<PracticeQuestionFormProps> = ({
  ...
  const validateForm = (): boolean => {
    ...
    if (formData.question_type === 'multiple_choice') {
      ...
      const optionTexts = options.map(opt => opt.option_text.trim().toLowerCase())
        ...
        <Box sx={{ display: 'flex', flexDirection: 'column', gap: 2 }}>
          ...
          <Box>
            <Button
              variant="outlined"
              component="label"
              startIcon={uploadingImage ? <CircularProgress size={20} /> : <ImageOutlined />}
          ...
          {formData.question_type === 'multiple_choice' && (
            <Paper sx={{ p: 2, bgcolor: 'background.default' }}>
              ...
              <Alert severity="info" sx={{ mt: 2 }}>
                Select one option as the correct answer. At least 2 options are required.
              </Alert>
            </Paper>
          )}
