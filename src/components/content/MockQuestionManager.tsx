...
} from '@/hooks/useMockQuestions'
import { MockQuestion, CreateMockQuestionInput } from '@/api/mockQuestions'

interface MockQuestionManagerProps {
    examPart: 'part_a' | 'part_b'
}

export const MockQuestionManager: React.FC<MockQuestionManagerProps> = ({
    examPart
}) => {
    ...
    const [formData, setFormData] = useState<CreateMockQuestionInput>({
        questionText: '',
    ...
    const { data: questions = [], isLoading } = useMockQuestions(examPart)
    const createMutation = useCreateMockQuestion()
    const updateMutation = useUpdateMockQuestion()
    const deleteMutation = useDeleteMockQuestion()
    ...
    return (
        <Box>
            ...
            {isLoading ? (
                ...
                <Alert severity="info">
                    No questions found for {examPart === 'part_a' ? 'Part A: Numeracy' : 'Part B: Clinical'}.
                    <br />
                        ...
                        {formData.questionType === 'multiple_choice' && (
                            <Box>
                                ...
                                {formData.options && formData.options.length > 0 ? (
                                    ...
                                    <Alert severity="warning">Add at least one option and mark the correct answer</Alert>
                                )}
                            </Box>
                        )}
