...
import { useLessonsByTopic, useCreateLesson, useUpdateLesson, useDeleteLesson } from '@/hooks/useLessons'
import { useTopics } from '@/hooks/useTopics'
import { PageLoader } from '@/components/common'
import { Lesson, CreateLessonInput, FIXED_MODULE_IDS } from '@/types/content'
import { getAllSubtopics, LEARNING_TOPICS } from '@/constants/learningStructure'
...
export const LessonManager: React.FC<LessonManagerProps> = ({ topicTitle, subtopicId }) => {
  ...
  const handleSubmit = async () => {
    ...
    try {
      if (editingLesson) {
        await updateMutation.mutateAsync({ id: editingLesson.id, input: formData })
      } else {
  ...
  const getLessonTypeIcon = (type?: string) => {
    switch (type) {
      case 'video': return <VideoLibraryOutlined fontSize="small" />
      case 'audio': return <AudioFileOutlined fontSize="small" />
      case 'quiz': return <QuizOutlined fontSize="small" />
      default: return <TextFieldsOutlined fontSize="small" />
    }
  }
            ...
            <TableBody>
              {lessons.map((lesson) => (
                <TableRow key={lesson.id}>
                  ...
                  <TableCell>
                    <Chip
                      icon={getLessonTypeIcon(lesson.lessonType)}
                  ...
                  <TableCell>
                    <Box sx={{ display: 'flex', gap: 0.5 }}>
                      {lesson.videoUrl && (
                        <Chip icon={<VideoLibraryOutlined fontSize="small" />} label="Video" size="small" />
                      )}
                      {lesson.audioUrl && (
                        <Chip icon={<AudioFileOutlined fontSize="small" />} label="Audio" size="small" />
                      )}
                    </Box>
                  </TableCell>
                  <TableCell>
                  ...
                  <TableCell>
                    <IconButton onClick={() => handleOpenDialog(lesson)} size="small">
                      <EditOutlined fontSize="small" />
                    </IconButton>
                    <IconButton onClick={() => handleDelete(lesson.id)} size="small" color="error">
