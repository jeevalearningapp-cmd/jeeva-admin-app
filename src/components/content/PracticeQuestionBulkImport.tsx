...
export const PracticeQuestionBulkImport: React.FC<PracticeQuestionBulkImportProps> = ({
  category,
  subdivision,
  onImportComplete
}) => {
      ...
      for (let i = 1; i < lines.length; i++) {
        ...
        try {
          const question: ParsedQuestion = {
            ...
            question_type: getColumnValue(headers, values, 'question_type') as any,
            ...
            correct_answer: parseInt(getColumnValue(headers, values, 'correct_answer')),
            is_active: getColumnValue(headers, values, 'is_active')?.toLowerCase() !== 'false'
          }
  ...
  const getColumnValue = (headers: string[], values: string[], columnName: string): string => {
              ...
              <Box>
                ...
                {invalidQuestions.length > 0 && (
                  <Typography variant="body2" color="error">
                    <strong>{invalidQuestions.length}</strong> questions with errors (will be skipped)
                  </Typography>
                )}
              </Box>
              <Button
        ...
        {activeStep === 2 && importResult && (
          <Box>
            <Alert
              ...
              icon={importResult.failed === 0 ? <CheckCircleOutlined /> : <ErrorOutlined />}
              sx={{ mb: 2 }}
            >
              <Typography variant="body1">
                Successfully imported <strong>{importResult.success}</strong> questions
              </Typography>
              {importResult.failed > 0 && (
                <Typography variant="body2">
                  Failed to import <strong>{importResult.failed}</strong> questions
                </Typography>
              )}
            </Alert>
