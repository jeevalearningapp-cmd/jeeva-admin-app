...
export const PracticeModuleTopicSelector: React.FC<
  PracticeModuleTopicSelectorProps
> = ({ onSelectionChange, selectedCategory, selectedSubdivision }) => {
  const [activeTab, setActiveTab] = useState<number>(0);
  const [questionCounts, setQuestionCounts] = useState<
    Record<string, QuestionCount[]>
  >({});
  ...
  const fetchQuestionCounts = async () => {
    try {
      ...
      const clinicalCounts = await Promise.all(
        PRACTICE_TOPICS.CLINICAL_KNOWLEDGE.subdivisions.map(
          async (subdivision) => {
            const { count, error } = await supabase
              .from("practice_questions")
              .select("*", { count: "exact", head: true })
  ...
  const handleTabChange = (_event: React.SyntheticEvent, newValue: number) => {
    ...
    const category =
      newValue === 0
        ? PRACTICE_TOPICS.NUMERACY.name
        : PRACTICE_TOPICS.CLINICAL_KNOWLEDGE.name;
  ...
  const getCurrentCategory = () => {
    return activeTab === 0
      ? PRACTICE_TOPICS.NUMERACY
      : PRACTICE_TOPICS.CLINICAL_KNOWLEDGE;
  };
  ...
  return (
    <Paper sx={{ p: 3 }}>
      <Box sx={{ display: "flex", alignItems: "center", mb: 3 }}>
        ...
        <Box>
          <Typography variant="h6">Practice Module Management</Typography>
            ...
            return (
              ...
              >
                <ListItemButton
                  selected={isSelected}
                  onClick={() =>
                    handleSubdivisionClick(currentCategory.name, subdivision)
                  }
                  sx={{
                    borderRadius: 1,
                    border: 1,
      ...
      {!loading && selectedCategory && selectedSubdivision && (
        <Alert severity="info" sx={{ mt: 2 }}>
          ...
          <strong>{selectedSubdivision}</strong>
        </Alert>
      )}
    </Paper>
  );
