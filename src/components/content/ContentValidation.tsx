...
export const ContentValidation: React.FC<ContentValidationProps> = ({ topic }) => {
  ...
  const validateCoreNotes = async (): Promise<ValidationResult> => {
    ...
    try {
      ...
      if (!coreNotes) {
        ...
        if (!coreNotes.content && (!coreNotes.sections || coreNotes.sections.length === 0)) {
  ...
  const validateFlashContent = async (): Promise<ValidationResult> => {
    ...
    try {
      ...
      if (flashScreens.length !== 5) {
        errors.push(`Flash Content must have exactly 5 screens (currently: ${flashScreens.length})`)
      } else {
    ...
    try {
      ...
      if (subtopicsData.length === 0) {
        ...
        for (const subtopic of subtopicsData) {
          const validation = await subtopicsAPI.getValidationStatus(subtopic.id)

          if (!validation.isValid) {
            errors.push(`Subtopic "${subtopic.title}": ${validation.errors.join(', ')}`)
          }

          if (!validation.hasVideo) {
            errors.push(`Subtopic "${subtopic.title}": Missing mandatory video lesson`)
          }
          ...
          if (!validation.hasPodcast) {
            warnings.push(`Subtopic "${subtopic.title}": No podcast (optional)`)
          }
        }
      }
    } catch (err) {
  ...
  if (!validation) {
    return (
      <Alert severity="error">
        Failed to validate topic content
      </Alert>
    )
  }
  ...
  return (
    <Box>
      ...
      <Alert
        ...
        icon={validation.overall.isValid ? <CheckCircleOutlined /> : <ErrorOutlined />}
        sx={{ mb: 3 }}
      >
      ...
      <Paper sx={{ border: '1px solid #E5E7EB', borderRadius: '16px', overflow: 'hidden' }}>
        <Box
          sx={{
            p: 2,
