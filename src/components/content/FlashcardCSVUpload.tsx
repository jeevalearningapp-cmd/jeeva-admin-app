...
export const FlashcardCSVUpload: React.FC<FlashcardCSVUploadProps> = ({
  category,
}) => {
...
"What is Duty of Candour?","A legal duty to be open and honest with patients when something goes wrong with their care.",""`;
  ...
  const handleBulkUpload = async () => {
    ...
    try {
      const flashcardsToCreate: CreateFlashcardInput[] = parsedFlashcards.map(
        (flashcard, index) => ({
          category,
      ...
      setSuccess(true);
      enqueueSnackbar(
        `${flashcardsToCreate.length} flashcards uploaded successfully`,
        { variant: "success" },
      );
      setParsedFlashcards([]);
    } catch (err: any) {
    ...
    <Box>
      <Box sx={{ mb: 3 }}>
        ...
        <Typography variant="body2" color="text.secondary" gutterBottom>
          Upload multiple flashcards at once using a CSV file. Download the
      ...
      <Alert severity="info" sx={{ mb: 3 }}>
        ...
        <ul style={{ margin: "8px 0 0 0", paddingLeft: "20px" }}>
          <li>
            <strong>front</strong> (required): The question or term
          </li>
          <li>
            <strong>back</strong> (required): The answer or definition
          </li>
          <li>
            <strong>image_url</strong> (optional): URL of an image
          </li>
        </ul>
      ...
      {success && (
        <Alert severity="success" sx={{ mb: 3 }} icon={<CheckCircleOutlined />}>
          Flashcards uploaded successfully! They are now available in the
          Flashcards tab.
        </Alert>
      )}
