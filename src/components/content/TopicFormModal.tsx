...
export const TopicFormModal: React.FC<TopicFormModalProps> = ({
  ...
  const handleClose = () => {
    if (!createMutation.isPending && !updateMutation.isPending && !isCreatingPlaceholders) {
      onClose()
    }
  }

  const isLoading =
    createMutation.isPending || updateMutation.isPending || isCreatingPlaceholders
  ...
  return (
    <Dialog open={open} onClose={handleClose} maxWidth="md" fullWidth>
          ...
          {!editingTopic && (
            <Alert severity="info">
              <Typography variant="body2">
                When you create a new topic, placeholders will be automatically created
                for:
              </Typography>
      ...
      <DialogActions>
        <Button onClick={handleClose} disabled={isLoading} sx={{ borderRadius: '12px' }}>
          Cancel
        </Button>
        <Button
