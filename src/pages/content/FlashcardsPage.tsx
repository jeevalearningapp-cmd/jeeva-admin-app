...
export const FlashcardsPage: React.FC = () => {
  ...
  const filteredFlashcards = flashcards?.filter((flashcard: Flashcard) => {
    ...
    const matchesLesson =
      lessonFilter === "all" || flashcard.lessonId === lessonFilter;
  ...
  const validate = () => {
    ...
    const hasAssociation =
      (formData.lessonId && formData.lessonId.trim() !== "") ||
  ...
  const getFieldError = (field: "lessonId" | "front" | "back") => {
    ...
    if (field === "lessonId" && !formData.lessonId && !formData.category)
      return "Either Lesson or Learning Topic is required";
    if (field === "front" && !formData.front.trim())
      return "Front text is required";
    if (field === "back" && !formData.back.trim())
              ...
              <TableRow key={flashcard.id}>
                ...
                <TableCell>
                  {flashcard.lessonId ? (
                    <Chip
                      label={
                        lessons?.find((l) => l.id === flashcard.lessonId)
                          ?.title || "Unknown"
                      }
                      size="small"
                    />
        ...
        <DialogContent>
          <Box sx={{ display: "flex", flexDirection: "column", gap: 2, mt: 2 }}>
            ...
            <Typography variant="body2" color="text.secondary" sx={{ mb: 1 }}>
              Flashcards can be associated with either a specific lesson OR a
            ...
            <FormControl fullWidth error={!!getFieldError("lessonId")}>
              ...
              <Typography
                ...
                sx={{ mt: 0.5, ml: 2 }}
              >
                For topic-level flashcards (e.g., all flashcards for "The NMC
                Code")
              </Typography>
            </FormControl>
