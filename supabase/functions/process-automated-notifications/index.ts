...
serve(async (req) => {
  try {
    ...
    if (automationConfig.subscription_expiring?.enabled) {
      const daysToCheck = automationConfig.subscription_expiring.days_before || [7, 3, 1]
        ...
        if (expiringSubscriptions && expiringSubscriptions.length > 0) {
          ...
          for (const sub of expiringSubscriptions) {
            ...
            const { data: existing } = await supabaseClient
              ...
              .eq('audience_filter', JSON.stringify({
                type: 'specific_users',
                userIds: [sub.user_id],
              }))
            ...
            if (existing && existing.length > 0) {
              console.log(`⏭️ Skipping duplicate notification for user ${sub.user_id}`)
              continue
            }
            ...
            if (!notificationError && notification) {
              ...
              console.log(`✅ Created subscription expiring notification for user ${sub.user_id}`)
            }
          }
        }
      }
    ...
    for (const milestone of milestones) {
      ...
      const { data: usersAtMilestone } = await supabaseClient.rpc(
      ...
      if (usersAtMilestone && usersAtMilestone.length > 0) {
        console.log(`🎯 Found ${usersAtMilestone.length} users who reached ${milestone} questions`)
    ...
    console.log(`✅ Automated notifications complete: ${notificationsCreated} created`)
