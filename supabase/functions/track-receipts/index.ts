...
serve(async (req) => {
  try {
    ...
    const { data: targets, error: targetsError } = await supabaseClient
      ...
      .select(
        "id, notification_id, push_token_id, expo_ticket_id, delivery_status",
      )
    ...
    const ticketIds = [
      ...new Set(targets.map((t) => t.expo_ticket_id).filter(Boolean)),
    ];
          ...
          if (receipt.status === "ok") {
            ...
            for (const target of matchingTargets) {
              ...
              if (
                ...
                console.log(
                  `⚠️ Marked token ${target.push_token_id} as inactive`,
                );
              }
    ...
    for (const notificationId of notificationIds) {
      ...
      if (!statsError && stats) {
        ...
        const failed = stats.filter(
          (s) => s.delivery_status === "failed",
        ).length;
    ...
    console.log(
      `✅ Receipt check complete: ${totalDelivered} delivered, ${totalFailed} failed`,
    );
