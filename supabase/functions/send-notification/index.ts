  ...
  try {
    ...
    for (const queueItem of queueItems) {
      try {
        ...
        if (!notification) {
          console.error(`❌ Notification not found for queue item ${queueItem.id}`)
          continue
        }
        ...
        const userIds = await getTargetUserIds(supabaseClient, notification.audience_filter)

        if (userIds.length === 0) {
          console.log(`⚠️ No target users found for notification ${notification.id}`)
        ...
        if (!pushTokens || pushTokens.length === 0) {
          console.log(`⚠️ No active push tokens found for notification ${notification.id}`)
        ...
        await supabaseClient.from('notification_targets').insert(targetsToInsert)
        ...
        console.log(`✅ Notification ${notification.id} processed: ${sent} sent, ${failed} failed`)
      } catch (error) {
    ...
    console.log(`✅ Processing complete: ${totalSent} sent, ${totalFailed} failed`)
...
async function getTargetUserIds(
  ...
  const filter = typeof audienceFilter === 'string' 
