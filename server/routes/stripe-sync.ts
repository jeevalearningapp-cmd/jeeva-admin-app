...
router.post("/payments", async (req: Request, res: Response) => {
  try {
      ...
      try {
        ...
        if (currencyConversion) {
          const sourceAmountCents =
            currencyConversion.amount_subtotal ||
            currencyConversion.amount_total;
          if (
            sourceAmountCents &&
        ...
        if (!validUserId) {
          console.log(
            `⚠️ Skipping session ${session.id}: No valid user_id in metadata`,
          );
          skipped++;
          continue;
        }
        ...
        if (!userExists) {
          console.log(
            `⚠️ Skipping session ${session.id}: User ${validUserId} not found in database`,
          );
          skipped++;
          continue;
        }
        ...
        if (insertError) {
          console.error(
            `Failed to import session ${session.id}:`,
            insertError.message,
          );
    ...
    console.log(
      `🏁 Sync complete: ${imported} imported, ${skipped} skipped, ${failed} failed`,
    );
...
router.get("/status", async (req: Request, res: Response) => {
  try {
    ...
    const completedSessions = recentSessions.data.filter(
      (s) => s.status === "complete",
    ).length;
