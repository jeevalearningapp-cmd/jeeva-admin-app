 ...
 */
router.get(
  "/topics/:topicId/validation-status",
    ...
    try {
      ...
      if (coreNotesError || !coreNotes) {
        ...
        validationWarnings.push(
          "Core Notes exist but are not marked as active",
        );
      }
      ...
      if (flashError) {
        ...
        validationErrors.push(
          `Exactly 5 Flash Content screens required (currently: ${flashContent?.length || 0})`,
        );
      } else {
        // Check each screen has content
        const emptyScreens = flashContent.filter(
          (screen) => !screen.content || screen.content.trim() === "",
        );
        if (emptyScreens.length > 0) {
          validationErrors.push(
            `Flash Content screens ${emptyScreens.map((s) => s.screen_number).join(", ")} have empty content`,
          );
        }

        // Check if all screens are active
        const inactiveScreens = flashContent.filter(
          (screen) => !screen.is_active,
        );
        if (inactiveScreens.length > 0) {
          validationWarnings.push(
            `Flash Content screens ${inactiveScreens.map((s) => s.screen_number).join(", ")} are not marked as active`,
          );
        }
      }
      ...
      if (subtopicsError) {
        ...
        for (const subtopic of subtopics) {
          ...
          if (lessonsError) {
            validationErrors.push(
              `Error fetching lessons for subtopic "${subtopic.title}"`,
            );
          } else if (!lessons || lessons.length === 0) {
            validationErrors.push(
              `Subtopic "${subtopic.title}" has no mandatory video lesson`,
            );
          }
          ...
          if (mcqError) {
            validationErrors.push(
              `Error fetching MCQs for subtopic "${subtopic.title}"`,
            );
          } else if (!mcqCount || mcqCount < 5) {
            validationErrors.push(
              `Subtopic "${subtopic.title}" has insufficient MCQs (${mcqCount || 0}/5 minimum)`,
            );
          } else if (mcqCount > 10) {
            validationWarnings.push(
              `Subtopic "${subtopic.title}" has more than 10 MCQs (${mcqCount}/10 maximum)`,
            );
          }
          ...
          if (unmappedError) {
            validationErrors.push(
              `Error checking MCQ mappings for subtopic "${subtopic.title}"`,
            );
          } else if (unmappedMCQs && unmappedMCQs.length > 0) {
            validationErrors.push(
              `Subtopic "${subtopic.title}" has ${unmappedMCQs.length} MCQs not mapped to video lessons`,
            );
          }
        }
      }
      ...
      res.json({
        ...
        data: {
          ...
          summary: {
            coreNotesValid: !validationErrors.some((e) =>
              e.includes("Core Notes"),
            ),
            flashContentValid: !validationErrors.some((e) =>
              e.includes("Flash Content"),
            ),
            subtopicsValid: !validationErrors.some(
              (e) => e.includes("Subtopic") || e.includes("subtopic"),
            ),
          },
        },
      });
      ...
      res.status(500).json({
        success: false,
        error:
          error instanceof Error ? error.message : "Failed to validate topic",
      });
    }
  },
);
 ...
 */
router.get(
  "/subtopics/:subtopicId/validation-status",
  async (req: Request, res: Response) => {
    try {
      ...
      if (lessonsError) {
        ...
        validationErrors.push(
          "At least one mandatory video lesson is required",
        );
      } else {
        // Check if video URL is provided
        const lessonsWithoutVideo = mandatoryLessons.filter(
          (l) => !l.video_url,
        );
        if (lessonsWithoutVideo.length > 0) {
          validationErrors.push(
            `${lessonsWithoutVideo.length} mandatory lesson(s) missing video URL`,
          );
        }
      }
      ...
      if (mcqError) {
        ...
        if (mcqCount < 5) {
          validationErrors.push(
            `Insufficient MCQs (${mcqCount}/5 minimum required)`,
          );
        } else if (mcqCount > 10) {
          validationWarnings.push(
            `More than 10 MCQs (${mcqCount}/10 maximum recommended)`,
          );
        }
        ...
        if (unmappedMCQs.length > 0) {
          validationErrors.push(
            `${unmappedMCQs.length} MCQ(s) not mapped to video lessons`,
          );
        }
      }
      ...
      res.json({
        ...
        data: {
          ...
          warnings: validationWarnings,
          summary: {
            mandatoryVideoValid: !validationErrors.some(
              (e) => e.includes("video") || e.includes("lesson"),
            ),
      ...
      console.error(
        "Error in GET /subtopics/:subtopicId/validation-status:",
        error,
      );
      res.status(500).json({
        success: false,
