...
/**
 * POST /api/notifications/direct-push
 * Send push notification directly to devices without storing in database
 */
router.post("/direct-push", async (req: Request, res: Response) => {
  try {
    const { title, body, imageUrl, data, audienceFilter } = req.body;
    ...
    console.log("📤 Direct push request received:", { title, audienceFilter });

    const result = await notificationService.sendDirectPush({
      title,
      body,
      imageUrl,
      data,
    ...
    res.json({
      success: true,
      message: `Direct push sent to ${result.sent} devices`,
      ...result,
    });
  } catch (error) {
    ...
    res.status(500).json({
      success: false,
      error:
        error instanceof Error ? error.message : "Failed to send direct push",
    });
  }
});
