    ...
    res.status(500).json({
      ...
      error:
        error instanceof Error
          ? error.message
          : "Failed to fetch topic flash content",
    });
  }
});
  ...
  try {
    ...
    res.status(500).json({
      ...
      error:
        error instanceof Error
          ? error.message
          : "Failed to create topic flash content",
    });
  }
});
 ...
 */
router.put(
  "/:topicId/flash-content/:screenNumber",
  async (req: Request, res: Response) => {
    try {
      ...
      console.error(
        "Error in PUT /:topicId/flash-content/:screenNumber:",
        error,
      );
      res.status(500).json({
        success: false,
        error:
          error instanceof Error
            ? error.message
            : "Failed to update topic flash content",
      });
    }
  },
);
...
router.delete(
  ...
  async (req: Request, res: Response) => {
    try {
      ...
      console.error(
        "Error in DELETE /:topicId/flash-content/:screenNumber:",
        error,
      );
      res.status(500).json({
        success: false,
        error:
          error instanceof Error
            ? error.message
            : "Failed to delete topic flash content",
      });
    }
  },
);
