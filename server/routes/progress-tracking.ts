 ...
 */
router.get('/:userId/subtopic-progress', async (req: Request, res: Response) => {
  try {
    const { userId } = req.params;
    ...
    res.status(500).json({
      success: false,
      error: error instanceof Error ? error.message : 'Failed to fetch subtopic progress',
    });
  }
});
 ...
 */
router.post('/:userId/subtopic-progress', async (req: Request, res: Response) => {
  try {
    const { userId } = req.params;
    const {
      topicId,
    ...
    if (existing) {
      ...
      if (timeSpentSeconds !== undefined) {
        updateData.time_spent_seconds = (existing.time_spent_seconds || 0) + timeSpentSeconds;
      }
    ...
    res.status(500).json({
      success: false,
      error: error instanceof Error ? error.message : 'Failed to update subtopic progress',
    });
  }
});
 ...
 */
router.get('/:userId/topics/:topicId/progress', async (req: Request, res: Response) => {
  try {
    ...
    const completedSubtopics = subtopicProgress?.filter(p => p.status === 'completed').length || 0;
    const coreNotesCompleted = topicProgress?.core_notes_completed || false;
    const flashContentCompleted = topicProgress?.flash_content_completed || false;
    ...
    res.status(500).json({
      success: false,
      error: error instanceof Error ? error.message : 'Failed to calculate topic progress',
    });
  }
});
