...
router.get('/presentment-summary', async (req: Request, res: Response) => {
  try {
    ...
    const { data: payments, error } = await supabase
      .from('payments')
      .select('currency_charged_local, amount_charged_local, amount_charged_gbp')
      .eq('gateway', 'stripe')
      .eq('status', 'succeeded')
    ...
    for (const payment of payments) {
      const currency = payment.currency_charged_local?.toUpperCase() || 'UNKNOWN'
    ...
    const byCurrency = Object.entries(currencyStats).map(([currency, stats]) => ({
      currency,
      count: stats.count,
      percentage: totalPayments > 0 ? Math.round((stats.count / totalPayments) * 10000) / 100 : 0,
      averageLocalAmount: stats.count > 0 ? Math.round((stats.totalLocalAmount / stats.count) * 100) / 100 : 0,
      averageGbpAmount: stats.count > 0 ? Math.round((stats.totalGbpAmount / stats.count) * 100) / 100 : 0,
...
router.get('/coupon-presentment', async (req: Request, res: Response) => {
  try {
    ...
    const { data: paymentsWithDiscounts, error: paymentsError } = await supabase
      .from('payments')
      .select('id, stripe_checkout_session_id, currency_charged_local, discount_amount, metadata')
      .eq('gateway', 'stripe')
      .eq('status', 'succeeded')
    ...
    for (let i = 0; i < paymentsWithDiscounts.length; i += batchSize) {
      ...
      await Promise.all(batch.map(async (payment) => {
        try {
          ...
          const totalDetailsDiscounts = (session as any).total_details?.breakdown?.discounts || []
          ...
          if (!couponCode && totalDetailsDiscounts.length > 0) {
            ...
            if (discount.discount?.coupon) {
              couponId = discount.discount.coupon.id || discount.discount.coupon
              couponCode = couponId
            }
          }
          ...
          if (couponCode && couponId) {
            const currency = payment.currency_charged_local?.toUpperCase() || 'GBP'
          ...
          console.warn(`⚠️ Could not retrieve session ${payment.stripe_checkout_session_id}:`, sessionError.message)
        }
      }))
    }
