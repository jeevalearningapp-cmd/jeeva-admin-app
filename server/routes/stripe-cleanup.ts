...
router.post("/recreate-plans", async (req: Request, res: Response) => {
  try {
    ...
    for (const plan of plans) {
      ...
      console.log(
        `  📊 Created 3 prices for ${plan.name}: INR ${plan.prices.INR}, USD ${plan.prices.USD}, GBP ${plan.prices.GBP}`,
      );
    }
    ...
    res.json({
      success: true,
      message:
        "✅ Successfully created 3 plans with exact pricing (9 prices total)",
      summary: {
