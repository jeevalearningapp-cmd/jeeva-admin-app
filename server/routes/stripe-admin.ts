...
router.post("/prices", async (req: Request, res: Response) => {
  try {
    ...
    if (requestedCurrency !== "GBP") {
      return res.status(400).json({
        error:
          "Only GBP prices are allowed. Adaptive Pricing handles currency conversion automatically.",
      });
    }
  ...
  try {
    ...
    const transformedPrices = prices.data
      ...
      .map((price, index) => ({
        ...
        plan_duration_days: parseInt(
          price.metadata?.plan_duration_days || "30",
        ),
      ...
      .sort((a, b) => {
        ...
        const aOrder =
          countryOrder[a.country_code as keyof typeof countryOrder] || 999;
        const bOrder =
          countryOrder[b.country_code as keyof typeof countryOrder] || 999;
...
router.post("/setup-plans", async (req: Request, res: Response) => {
  try {
    const plans = [
      {
        name: "Starter",
        description:
          "Perfect for beginners - 30 days of access to all learning materials",
        days: 30,
        priceINR: 3000,
        priceUSD: 34,
        priceGBP: 25,
      ...
      },
      {
        name: "Growth",
        description:
          "Accelerated learning - 90 days comprehensive study plan with advanced features",
        days: 90,
        priceINR: 8000,
        priceUSD: 90,
        priceGBP: 68,
      ...
      },
      {
        name: "Ultimate",
        description:
          "Complete mastery - 150 days intensive preparation with premium support",
        days: 150,
        priceINR: 15000,
        priceUSD: 168,
        priceGBP: 127,
    ...
    res.json({
      success: true,
      message:
        "All 3 plans created successfully with 3 currencies each (9 prices total)",
      plans: results,
    });
  } catch (error: any) {
