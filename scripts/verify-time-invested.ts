...
async function verifyTimeInvested() {
  ...
  if (userError || !users.users.length) {
    console.log(
      "⚠️ Could not list users (likely need Service Role Key). Trying with anon login...",
    );
  ...
  if (questionId) {
    ...
    console.warn(
      "⚠️ Still no question ID available. Skipping result insertion.",
    );
  }
  ...
  if (!finalAnalytics) {
    console.error(
      "❌ No analytics record found! Trigger might not be working.",
    );
  } else {
    ...
    if (
      finalAnalytics.time_spent_minutes >
      (initialAnalytics?.time_spent_minutes || -1)
    ) {
      ...
      console.warn(
        "⚠️ Potential Issue: migration might not update time_spent_minutes column on UPDATE.",
      );
      console.log(
        "Value of average_time_seconds:",
        finalAnalytics.average_time_seconds,
      );
    }
